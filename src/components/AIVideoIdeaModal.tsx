...
const AIVideoIdeaModal = () => {
  ...
  // --- STREAMING EFFECT KE LIYE useEffect ---
  useEffect(() => {
    if (results) {
      let i = 0;
      setDisplayedResults('');
      const interval = setInterval(() => {
        if (i < results.length) {
          // Ek saath 5 character add karenge
          const chunk = results.substring(i, i + 5);
          setDisplayedResults((prev) => prev + chunk);
          i += 5;
        } else {
          clearInterval(interval);
        }
      }, 10); // Interval 10ms hi rahega, lekin ab 5 guna tez hoga
      ...
      <DialogContent className="glass-card max-w-2xl max-h-[90vh] overflow-y-auto border-purple-500/30 shadow-2xl shadow-purple-500/20">
        <div className="space-y-6 p-6">
          ...
          <div className="space-y-4">
            {/* Form fields same rahenge... */}
            <div className="space-y-2">
              <label htmlFor="topic" className="text-sm font-medium">Your Video Topic</label>
              <Input id="topic" placeholder="e.g., Travel vlog, Product review, Tutorial..." value={formData.topic} onChange={(e) => setFormData({ ...formData, topic: e.target.value })} className="w-full bg-background/50 border-purple-500/30" />
            </div>
            <div className="space-y-2">
              <label htmlFor="style" className="text-sm font-medium">Video Style</label>
              <Select onValueChange={(value) => setFormData({ ...formData, style: value })}>
                <SelectTrigger className="bg-background/50 border-purple-500/30"><SelectValue placeholder="Choose a video style" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={(value) => setFormData({ ...formData, audience: value })}>
                <SelectTrigger className="bg-background/50 border-purple-500/30"><SelectValue placeholder="Select your target audience" /></SelectTrigger>
                <SelectContent>
            ...
            <Button onClick={handleGenerateIdeas} disabled={isGenerating} className="btn-primary w-full">
              {isGenerating ? 'Generating Ideas...' : 'Generate Ideas'}
            </Button>
          </div>

          {/* Results Area */}
          {(isGenerating || displayedResults) && (
            <div className="space-y-4 pt-4 border-t border-border/20">
              ...
              <div className="glass-card p-4 rounded-lg border border-border/20 text-left">
                {/* Ab hum 'displayedResults' ko dikhayenge */}
                <pre className="text-sm leading-relaxed whitespace-pre-wrap font-poppins">
                  {displayedResults}
                  {isGenerating && !displayedResults && "Generating..."}
                </pre>
              </div>
            </div>
          )}

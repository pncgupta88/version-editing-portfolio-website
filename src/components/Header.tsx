  ...
  return (
    <header className={`h-[80px] fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
      ...
      : 'bg-transparent'
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && (
          <nav className="md:hidden mt-4 py-4 px-4 border-t border-purple-500/20 bg-background/90 backdrop-blur-md fixed top-[63.5px] left-81 right-0 z-40">
            <div className="flex flex-col space-y-4">
              {navItems.map(item => (
                <button
                  key={item.label}
                  onClick={() => scrollToSection(item.href)}
                  className="text-right text-foreground/80 hover:text-purple-400 transition-colors duration-300 font-medium"
                >
                  {item.label}
                </button>
              ))}
